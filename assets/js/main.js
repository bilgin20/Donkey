...
const DAILY_REWARD = 1000; // Günlük ödül puanı
const lastLoginDate = localStorage.getItem('lastLoginDate');
const currentDate = new Date().toDateString(); // Günlük kontrol için tarih

// Günlük giriş ödül kontrolü ve ekleme
if (lastLoginDate !== currentDate) {
    total = total ? Number(total) + DAILY_REWARD : DAILY_REWARD;
    power = power ? Number(power) + DAILY_REWARD : DAILY_REWARD;

    localStorage.setItem('total', total);
    localStorage.setItem('power', power);
    localStorage.setItem('lastLoginDate', currentDate); // Giriş tarihini güncelle
}
